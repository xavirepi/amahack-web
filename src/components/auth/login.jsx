...
const validators = { // We create an object that has a key for each form field 
    email: (value) => { // Each key has a function as a value that will check whether there are errors or not
        let message = null; // Initalized as undefined
...
const Login = () => { // The actual component to be returned
    ...
    const onSubmit  = (e) => {
        ...
        if (isValid()) { // If the form isn't valid we won't be making the axios request
            ...
            login(state.fields) // We use login method from /services/Authservice.js and pass state.fields as an argument
    ...
    const { errors } = state.fields // We bring the errors so we can set a conditional to render the invalid feedback
